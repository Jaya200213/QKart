  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={(e)=>{addAddress(token,newAddress.value);
          ...
          <Box className="shipping-container" minHeight="100vh">
...

            {
              (addresses.all.map((add)=>
              {
                ...
                return(
                  <div className={classname} onClick={(e)=>{setAddresses({...addresses,selected:add["_id"]})}}  key={add["_id"]}>
                    <p my="1rem" >{add["address"]}</p>
                    <Button startIcon={<Delete />} id={add["_id"]} onClick={(e)=>{deleteAddress(token,e.target.id)}}>
                      DELETE
                    </Button>
                  </div>)
              }))
